...
class PassiveHandEnv(robot_env.RobotEnv):
    ...
    def __init__(
        ...
        super(PassiveHandEnv, self).__init__(
    ...
    def _set_action(self, action):
        ...
        vert_angle = action[3]
        hor_angle = -np.arctan2(*(self.sim.data.mocap_pos[0][:2] - np.asarray([0.725, 0.74910034]))) + np.pi / 2
        ...
        # rot_ctrl = [1., 0., 1., 1.]  # fixed rotation of the end effector, expressed as a quaternion
    ...
    def _get_obs(self):
        ...
        if self.has_object:
            ...
            object_pos = object_velp = object_velr = object_rel_pos = np.zeros(0)
        ...
        obs = np.concatenate([
            grip_pos, object_pos.ravel(), object_rel_pos.ravel(), grip_rot.ravel(),
    ...
    def _reset_sim(self):
        ...
        #     object_xpos = self.initial_gripper_xpos[:2] + np.asarray([0.1, 0])
    ...
    def _sample_goal(self):
        ...
        #     goal = self.initial_gripper_xpos[:3] + self.np_random.uniform(-self.target_range, self.target_range, size=3)
        # return goal.copy()
        return np.asarray([1.46177789, 0.74909766, 0.7])
    ...
    def render(self, mode='human', width=500, height=500):
        return super(PassiveHandEnv, self).render(mode, width, height)
