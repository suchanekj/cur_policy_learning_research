# By Adam Sroka 20 Nov 2019
# Very much inspired by Stan
...
if not args.get("video", False):
    vs = VideoStream(src=camera_num).start()
...
while True:
    ...
    # Moving the robot
    if center:
        print('x is {}, y is {}'.format(center[0], center[1]))
        x_curr = center[0]
        y_curr = center[1]
        move_x = - x_centre + x_curr
        move_y = y_centre - y_curr

        # Move robot
        robot_arm.write(str(move_x)+' '+str(move_y)+' 0 '+'MOVE')
        print(robot_arm.read())
    ...
    has_already_started, start_time, frame_counter, frame_rate = get_framerate(
        has_already_started, start_time, frame_counter, frame_rate)

    write_framerate(frame, frame_rate, (30, 30))
    ...
    cv2.resizeWindow('Frame', window_res)
